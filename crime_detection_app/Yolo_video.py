...
def video_detection(path_x, callback_function=None):
    ...
    custom_weights_path = '../YOLO-Weights/best.pt'

    # Initialize YOLO model with custom weights
    model = YOLO(custom_weights_path)

    classNames = ['burglary', 'fighting', 'robbery']
    if classNames in ["fighting", "burglary", "robbery"]:
        message = f"{classNames.capitalize()} detected at student gate!"
